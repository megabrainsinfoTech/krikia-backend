...
export class AuthService {
    ...
    async signup(signupBody: CreateUserDTO){
        ...
        if(!!doesExist) {
            throw new HttpException(`Sorry. ${signupBody.email} has already been registered`, HttpStatus.CONFLICT);
        }
        ...
        return {message: `Account created successfully. Verify your email and login`};
        ...
        try {
            ...
            if (!!email) {
                ...
                if (!!user) {
                    ...
                    user.set({status: "Active", avatarUrl: `${process.env.CDN_URL}/krikia-assets/default_avatar.png`});
                    user.save();
    ...
    async login(loginBody: LoginAuthDTO): Promise<ILoginPayload> {
        ...
        if(!user){
            throw new HttpException("Sorry, this account does not exist", HttpStatus.NOT_FOUND);
        }

        if(user.status === "Pending"){
            throw new HttpException("Go to your email to verify new user account", HttpStatus.FORBIDDEN);
        }

        // Password
        const isMatch = await user.cmpPassword(loginBody.password);
        if(!isMatch){
            throw new HttpException("Incorrect login credentials", HttpStatus.BAD_REQUEST);
        }
        ...
        return { "__uAud": encodeBase64Url(`${user.id},${user.email}`), userId: user.id , "__ASSEMBLYsxvscz090619_": accessToken as string };
    }
    ...
    async createBusiness(createBusinessBody: CreateBusinessDTO, userId: string){
        ...
        if(!!userId){
            try {
                ...
                await this.userBusinessService.create({businessId: business.id, userId, role: "Creator"});

                // Create wallet for business
                const wallet = (await apiConnect(process.env.PAYMENT_SERVICE_URL as string).post("wallet/create", { holderEmail: business.email, type: "Business" })).data;
                return wallet;
        
            } catch(err){
                if(err.code === "ECONNREFUSED") throw new HttpException("Can't create wallet now. Try again later", HttpStatus.
                SERVICE_UNAVAILABLE)
                else throw new HttpException(err.message, HttpStatus.INTERNAL_SERVER_ERROR)
            }
        }

        return { pending: !userId, businessId: business.id, business: { businessName: business.name, businessAlias: business._alias } };
    }
    ...
    async createBusinessComplete(businessId: string, userId: string){
        ...
        try {
            ...
            await this.userBusinessService.create({businessId, userId, role: "Creator"});
