...
import { CreateUserDTO, PayForListingDTO, UpdateUserDTO } from './user.dto';
...
export class UserService {
    ...
    async create(createBody: CreateUserDTO){
        await this.user.create({...(createBody as any), status: "Pending"});
    }
    ...
    async update(updateBody: UpdateUserDTO){
        await this.user.update(updateBody, { where: { id: this.cls.get("userId") } });
    ...
    async contentStats(){
        const stats = await this.user.sequelize?.query(loadSQL("user-contents-stats"), {
    ...
    purchase(purchaseId: string){
        return this.purchaseService.findOneByUserId(this.cls.get("userId"), purchaseId);
    }

    async pay(userPayBody: PayForListingDTO){
        try {
            const { amount, totalAmount, paymentFrequency, listingPlanOptionId, fnxReceiverAddress, purchaseId } = userPayBody;
            const user = await this.user.findOne({ where: { id: this.cls.get("userId") }, attributes: ["fnxAccountAddress"] });
            ...
            const listingPlanOption = await this.listingPlanOptionService.findById(listingPlanOptionId);
            if(purchase){
                purchase.set({ paidAmount: purchase.paidAmount+amount });
                purchase.save();
            } else {
                purchase = await this.purchaseService.create({ paidAmount: amount, totalAmount, paymentFrequency: paymentFrequency, listingPlanOptionId, userId: this.cls.get("userId"), paymentDuration: listingPlanOption?.paymentDuration || 0 });
            }
    ...
    async createWallet(userEmail: string){
        try {
            ...
            const wallet = (await apiConnect(process.env.PAYMENT_SERVICE_URL as string).post("wallet/create", { holderEmail: userEmail, type: "User" })).data;
            return wallet;
    ...
    async getWallets(userEmail: string){
        try {
            const wallets = (await apiConnect(process.env.PAYMENT_SERVICE_URL as string, { Authorization: `AppAccess${userEmail}`}).get("wallet")).data;
            return wallets;
    ...
    async createWishlistItem(createWishlistBody: CreateWishlistDTO){
        await this.wishlistService.create(this.cls.get("userId"), createWishlistBody);
    ...
    async getWishlist(){
        return await this.listingService.findWishlistByUserId(this.cls.get("userId"));
    }

    async getMiniWishlist(){
        return await this.listingService.findMiniWishlistByUserId(this.cls.get("userId"));
    }

    async createSharedWishlist(createSharedWishlistBody: CreateSharedWishlistDTO){
